...
class FirecrawlApp:
    ...
    def scrape_url(self, url: str, params: Optional[Dict[str, Any]] = None) -> Any:
        ...
        if response.status_code == 200:
            ...
            if response['success'] and 'data' in response:
                ...
                raise Exception(f'Failed to scrape URL. Error: {
                                response["error"]}')
            else:
    ...
    def crawl_url(self, url: str,
        ...
        response = self._post_request(
            f'{self.api_url}{endpoint}', json_data, headers)
        if response.status_code == 200:
    ...
    def cancel_crawl(self, id: str) -> Dict[str, Any]:
        ...
        """
        headers = {
            # {"Content-Type": "application/json"}  # self._prepare_headers()
            'Content-Type': "application/json"}
        response = self._delete_request(
            f'{self.api_url}/v1/crawl/{id}', headers)
        if response.status_code == 200:
            return response.json()
        else:
    ...
    def map_url(self, url: str, params: Optional[Dict[str, Any]] = None) -> Any:
        ...
        if response.status_code == 200:
            ...
            if response['success'] and 'links' in response:
                ...
                raise Exception(f'Failed to map URL. Error: {
                                response["error"]}')
            else:
    ...
    def batch_scrape_urls_and_watch(self, urls: list[str], params: Optional[Dict[str, Any]] = None, idempotency_key: Optional[str] = None) -> 'CrawlWatcher':
        ...
        """
        crawl_response = self.async_batch_scrape_urls(
            urls, params, idempotency_key)
        if crawl_response['success'] and 'id' in crawl_response:
            return CrawlWatcher(crawl_response['id'], self)
        else:
        ...
        while True:
            ...
            if status_response.status_code == 200:
                ...
                if status_data['status'] == 'completed':
                    ...
                    raise Exception(f'Crawl job failed or was stopped. Status: {
                                    status_data["status"]}')
            else:
    ...
    def _handle_error(self, response: requests.Response, action: str) -> None:
        ...
        if response.json():
            ...
            error_details = response.json().get(
                'details', 'No additional error details provided.')

        if response.status_code == 402:
            message = f"Payment Required: Failed to {
                action}. {error_message} - {error_details}"
        elif response.status_code == 408:
            message = f"Request Timeout: Failed to {
                action} as the request timed out. {error_message} - {error_details}"
        elif response.status_code == 409:
            message = f"Conflict: Failed to {action} due to a conflict. {
                error_message} - {error_details}"
        elif response.status_code == 500:
            message = f"Internal Server Error: Failed to {
                action}. {error_message} - {error_details}"
        else:
            message = f"Unexpected error during {action}: Status code {
                response.status_code}. {error_message} - {error_details}"
...
class CrawlWatcher:
    ...
    async def _handle_message(self, msg: Dict[str, Any]):
        if msg['type'] == 'done':
            ...
            self.dispatch_event(
                'done', {'status': self.status, 'data': self.data})
        elif msg['type'] == 'error':
            self.status = 'failed'
            self.dispatch_event(
                'error', {'status': self.status, 'data': self.data, 'error': msg['error']})
